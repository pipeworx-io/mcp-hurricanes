...
`tools/list` at `https://gateway.pipeworx.io/hurricanes/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/active_storms \
  -H 'Content-Type: application/json' \
  -d '{"basin":"atlantic"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/active_storms`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
